...
class PerlmutterHvp(object):
    ...
    def update_opt(self, f, target, inputs, reg_coeff, name=None):
        ...
        with tf.name_scope(name, "update_opt", [f, inputs, params]):
...
class FiniteDifferenceHvp(object):
    ...
    def update_opt(self, f, target, inputs, reg_coeff, name="update_opt"):
        ...
        with tf.name_scope(name, "update_opt", [f, inputs, params, target]):
            constraint_grads = tf.gradients(
                f, xs=params, name="gradients_constraint")
            for idx, (grad, param) in enumerate(zip(constraint_grads, params)):
                if grad is None:
            ...
            def f_hx_plain(*args):
                with tf.name_scope(
                        "f_hx_plain", values=[inputs, self.target, xs]):
...
class ConjugateGradientOptimizer(Serializable):
    ...
    def update_opt(self,
        ...
        with tf.name_scope(
                name, "update_opt",
                [loss, target, leq_constraint, inputs,
                 extra_inputs]):  # yapf: disable
            inputs = tuple(inputs)
